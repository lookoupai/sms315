...
export default function CountryDetailPage() {
    ...
    const fetchCountryData = async () => {
      ...
      try {
        ...
        if (countryData) {
          ...
          setError(t('detail.countryNotFound'))
        }
      } catch (err) {
        console.error('获取国家详情失败:', err)
        setError(t('detail.fetchCountryFailed'))
      } finally {
        setLoading(false)
      }
    }
  ...
  useEffect(() => {
    if (country) {
      const localizedCountryName = getLocalizedCountryName(country.name, i18n.language)
      const title = `${localizedCountryName} (${country.code}) - ${t('detail.countryDetailTitle')}`
      const description = `查看${localizedCountryName}的所有接码记录，包括成功和失败的案例。${t('detail.phoneCode')}${country.phone_code}`
      ...
      const structuredData = {
        ...
        "mainEntity": {
          "@type": "Country",
          "name": getLocalizedCountryName(country.name, i18n.language),
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">{t('detail.loadingCountry')}</p>
        </div>
      </div>
    )
  }
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 flex items-center justify-center">
        <Card className="max-w-md">
          <CardContent className="pt-6 text-center">
            ...
            <p className="text-gray-600 mb-4">{error || t('detail.countryNotFound')}</p>
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 py-8">
      <div className="max-w-6xl mx-auto px-4">
        ...
        <nav className="flex items-center space-x-2 text-sm mb-6">
          ...
          <span className="text-gray-600">{t('detail.countryDetail')}</span>
          <span className="text-gray-400">/</span>
          <span className="text-gray-900 font-medium">{getLocalizedCountryName(country.name, i18n.language)}</span>
        </nav>
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle className="flex items-center gap-2">
                ...
                <span>{getLocalizedCountryName(country.name, i18n.language)} ({country.code})</span>
              </CardTitle>
            </div>
          </CardHeader>
          <CardContent>
            <div className="mb-4">
              <span className="font-medium text-gray-700">{t('detail.phoneCode')}</span>
