...
function extractCountryCode(countryName: string): string | null {
  ...
  // 3. 如果是纯英文名称，尝试通过反向查找所有可能的ISO代码
  if (/^[a-zA-Z\s\-'\.]+$/.test(countryName)) {
    // 遍历所有可能的ISO代码，看哪个能匹配当前名称
    const allIsoCodes = [
    ...
    // 尝试每个ISO代码，看是否能生成匹配的英文名称
    for (const code of allIsoCodes) {
      try {
        if (typeof Intl !== 'undefined' && Intl.DisplayNames) {
          const displayNames = new Intl.DisplayNames(['en-US'], { type: 'region' })
          const englishName = displayNames.of(code)
          if (englishName && englishName.toLowerCase() === countryName.toLowerCase()) {
            return code
          }
        }
      } catch (error) {
